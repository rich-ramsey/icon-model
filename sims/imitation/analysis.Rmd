...
# load("sims/imitation/.RData") # test RW's prior data
head(out_imi_basic)
...
Note to Rob - data for the distractor feat_array and identity parameters are identical for timecourse activation plots. Does this make sense?
...
Note to Rob - both of these ROC plots are different to the plots in the main text (IMITATE.ROC). They are different if I plot using your code and also if I wrangle and plot using ggplot/tidyverse. Which is right? The figure in the main text or the plots here?
...
ggsave ("sims/imitation/figures/timecourse_roc.jpeg")
```
...
# load("sims/imitate/.RData") # test RW's prior data
head(out_imi_lo)
...
(with bias on the x-axis)

```{r}
p10.3 <- ggplot(rts_diff, aes(x = bias, y = ce)) +
  geom_violin() +
...
ggsave ("sims/imitation/figures/ce_cycles_all.jpeg")
```

with bias as a facet_wrap (to help with a combined plot)
...
ggsave ("sims/imitation/figures/rt_ce_cycles_all.jpeg")
```
...
ggsave ("sims/imitation/figures/time_rt_ce_all.jpeg")
```
