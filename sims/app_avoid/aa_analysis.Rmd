...
plot sims from stim damage (at a lower level than earlier stim damage) plus increased global noise sims.
...
rts_all <- rbind(rts_aa, rts_aa_stim_10, rts_aa_global, rts_aa_stim_global) %>%
  select(simno, task, noise, acc, cycle) %>% 
  mutate(task = factor(task, levels = c("congr", "incon")),
         noise = factor(noise, levels = c("baseline", "stimulus", "global", "stimulus_global"))) %>% 
  filter(acc==1)
head(rts_all)
# summary(rts_all)
```
...
```{r}
p13 <- ggplot(rts_all, aes(x = task, y = cycle, fill = task)) +
  geom_violin() +
  geom_jitter(position=position_jitterdodge(dodge.width =1), alpha = 0.8, colour = "darkgrey") +
  geom_point(data=rts_all_summary, aes(y=mean_rt), size =3) +
  geom_errorbar(data=rts_all_summary, aes(y = mean_rt, ymin=mean_rt-sem, ymax=mean_rt+sem), width=.2) +
